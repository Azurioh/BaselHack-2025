...
const navbarTextStyle = (path: string) =>
  `text-black font-medium text-lg hover:text-gray-800 px-6 py-3 rounded-lg transition-all duration-200 scale-100 hover:scale-110 ${location.pathname === path ? 'underline underline-offset-8' : ''} font-semibold`;
    ...
    <nav className="bg-white border-b border-gray-200 px-6 h-16 flex items-center justify-between">
      ...
      <div className="flex items-center gap-4 !pr-8">
        <Dropdown menu={{ items: userMenuItems }} placement="bottomRight" trigger={['click']}>
          <button
            className="flex items-center gap-2 text-text hover:bg-background px-3 py-2 rounded-lg transition-colors font-semibold !p-2"
            style={{ fontFamily: 'var(--font-body)' }}>
